...
| 610 | Query parameter does not exist | In the event where you manufacture your own query params and expect DevLess to understand them I will like to saw you are too kind. [List of query params](#) |
...
| 617 | No such table belongs to the service | When the name of the have provided does not belong to the specified service |
...
| 634 | Seems the table does not exist | When you try to perform an operation on a table and the table does not exist 😂😂😂. . Any way could be that you spelt it wrongly though |
| 635 | Sorry to use offset, you need to set size | So if you decide to get you data from an offset you will need to set the number of records you want as well . Well this forces you to paginate |
...
| 640 | Sorry, there are no such related tables | When you lie to DevLess telling it table A is related to b and that it should get data from b this what you get |
| 641 | Something is wrong with your payload | When you are about to add data via JSON and you payload is all wrong .Look up the right way from here. |
...
| 645 | Sorry but you are not logged in as Admin | Something are for Admin only.🙂 . Get over it. |
...
| 700 | Internal system error | This captures all un forseen errors . Sometimes errors from within the core of the framework|
